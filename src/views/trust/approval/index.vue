<template>
    <div class='approval'>
          ...
          <div class='menu-search'> 
              <div class='menu-title'>
                  ...
                  <div class='searchItem'>
                      ...
                      <el-select v-model='listQuery.delStatus' size='small'>
                          <el-option
              ...
              <div class='menu-title'>
                  ...
                  <div class='searchItem searchItem-date'>
                      <span>{{$t('approval.checkDate')}}</span>
                      <el-date-picker
                      size='small'
                      class="dates"
      ...
      <el-card class='box-card' style="margin-top: 10px;margin-bottom: 20px;">
        ...
        <div class='menu-list'>
            <button type='button' :disabled='isDisable2' @click='approvalFn'>{{$t('approval.appr')}}</button>
            <button type='button' @click='receive'>{{$t('approval.task')}}</button>
        </div>
        <el-table
            ...
            width='130'>
            <template scope="scope">
                <span v-if="scope.row.checkUserStatus=== 0">{{$t('approval.noappr')}}</span>
                <span v-else-if="scope.row.checkUserStatus=== ''">{{$t('approval.noappr')}}</span>
                <span v-else-if="scope.row.checkUserStatus=== null">{{$t('approval.noappr')}}</span>
                <span v-else>{{$t('approval.handle')}}</span>
            </template>
            </el-table-column>
            <el-table-column
            prop='checkOpinion'
...
import { getTask, page } from 'api/trust/approval/index'
export default {
  name: "approval",
  data() {
    return {
      ...
      status: [
        ...
        {
          value: 1,
          label: this.$t('approval.signin')
        },
        {
          value: 2,
          label: this.$t('approval.handle')
        }
      ],
      isDisable2: true
    };
  ...
  computed: {
    ...mapGetters(["elements", "language", "approvalPage"])
  },
  created(){
    if (this.approvalPage.has(this.$route.path)) {
      this.listQuery = this.approvalPage.get(this.$route.path)
    }
    this.getList();
  },
  methods: {
    ...
    //审批时间
    getSTimeStart2(val) {
      this.listQuery.checkBeginDate = val;
    },
    //申请结束时间
    getSTimeEnd2(val) {
    ...
    getList() {
      ...
      this.$store.dispatch('saveApprPage', { path: this.$route.path, listQuery: this.listQuery });
    ...
    receive() {
      ...
      getTask()
        .then(response => {
          ...
          if (response.data.apply === 0) {
            this.$message({
              showClose: true,
              message: this.$t('approval.error'),
              type: 'warning'
            });
            this.getList()
          } else if (response.data.apply === 1) {
            this.$message({
              showClose: true,
            ...
            this.getList()
          } else if (response.data.apply === 2) {
            this.$message({
              showClose: true,
              message: this.$t('approval.most'),
              type: 'warning'
            });
            this.getList()
          }
    ...
    handleSelectionChange(val) {
      ...
      if (this.multipleSelection.length !== 0) {
        if (this.multipleSelection[0].checkUserStatus === 0 || this.multipleSelection[0].checkUserStatus === null || this.multipleSelection[0].checkUserStatus === '') {
    ...
    approvalFn() {
      if(this.multipleSelection.length == 1) {   
        this.$store.dispatch('setApprDetail', this.multipleSelection[0])
        this.$router.push({
