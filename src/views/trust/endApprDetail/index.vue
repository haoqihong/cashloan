                    ...
                    <el-card class="box-card" style="margin-bottom: 8px;">
                        ...
                        <div class='personal-opinion'>
                            <div class='personal-opinion-item'>
                                ...
                                <el-select v-model='telStatus' :placeholder="$t('rule.choose')" size='small' :disabled="true">
                                    <el-option
                            ...
                            <div class='personal-opinion-item'>
                                ...
                                <el-input type='textarea' v-model='telOpinion' :placeholder="$t('rule.toLength')" :maxlength="500" :disabled="true"></el-input>
                            </div>
                            <div class='personal-opinion-item'>
                                <span><i style="color: red;margin-right: 2px;">*</i>{{$t('approvalDetail.result')}}</span>
                                <el-radio-group v-model='status' @change="changeHandler" style="margin: 10px 0;" :disabled="true">
                                ...
                                <div class='personal-info-item' v-show="isQuota">
                                    ...
                                    <el-input v-model='approvedAmt' size='mini' :disabled="true"></el-input>
                                </div>
                            </div>
                            <div class='personal-opinion-item'>
                                <span>{{$t('approvalDetail.remarks')}}</span>
                                <el-input type='textarea' v-model='checkOpinion' :placeholder="$t('rule.toLength')" :maxlength="500" :disabled="true"></el-input>
                            </div>
                            <div class='personal-opinion-item'>
                                <span><i style="color: red;margin-right: 2px;">*</i>{{$t('approvalDetail.finaljud')}}</span>
                                <el-radio-group v-model='status1' @change="changeHandlerEnd" style="margin: 10px 0;">
                                    <el-radio :label="$t('approvalDetail.adopt')" size='mini'></el-radio>
                                    <el-radio :label="$t('approvalDetail.refuse')" size='mini'></el-radio>
                                    <el-radio :label="$t('approvalDetail.derate')" size='mini'></el-radio>
                                </el-radio-group>
                                <div class='personal-info-item' v-show="isQuota1">
                                    <span><i style="color: red;margin-right: 2px;">*</i>{{$t('approvalDetail.limit')}}</span>
                                    <el-input type="number" v-model='approvedAmtEnd' size='mini' @input="blurFnEnd"></el-input>
                                </div>
                            </div>
                            <div class='personal-opinion-item'>
                                <span>{{$t('approvalDetail.finalremarks')}}</span>
                                <el-input type='textarea' v-model='checkOpinion1' :placeholder="$t('rule.toLength')" :maxlength="500"></el-input>
                            </div>
                        </div>
                    </el-card>
                    <div class="back">
...
<script>
    ...
    import { getInfo, getContact, getHit, getImg, getCustomerInfo, getAppInfo, getApproInfo, finalApprovel, getResultById, hisPage, getPercent, resolveGps } from 'api/trust/approvalDetail/index';
    ...
    export default {
        ...
        created() {
            ...
            getApproInfo(objs) 
            .then(response => {
                this.telStatus = response.data.telStatus;
                this.telOpinion = response.data.telOpinion;
                this.checkOpinion = response.data.checkOpinion;
                if(response.data.checkUserStatus === 1) {
                    this.status = this.$t('approvalDetail.adopt')
                } else if(response.data.checkUserStatus === 2) {
                    this.status = this.$t('approvalDetail.refuse')
                } else if(response.data.checkUserStatus === 3) {
                    this.status = this.$t('approvalDetail.derate')
                    let re = /(?=(?!\b)(\d{3})+$)/g;
                    let str = String(response.data.approvedAmt)
                    response.data.approvedAmt =  str.replace(re, '.');
                    this.approvedAmt = response.data.approvedAmt
                }
            })
        },
        computed: {
            ...mapGetters(["elements", "language", "apprDetailsEnd"])
        },
        mounted () {
            this.viewer()
        },
        methods: {
            ...
            getParamsDetail () {
                this.orderId = this.apprDetailsEnd.orderId;
                this.imgQuery.userId = this.apprDetailsEnd.userId;
                this.imgQuery.channel = this.apprDetailsEnd.channel;
                this.prodId = this.apprDetailsEnd.prodId;
            },
            //查看认证结果
            seeResult () {
                let obj = {};
            ...
            submit() {
                if(this.status1 === "") {
                    this.$message({
                        showClose: true,
                        message: this.$t('approvalDetail.cue6'),
                        type: 'warning'
                    });
                } else if(this.status1 === this.$t('approvalDetail.derate') && this.approvedAmtEnd === '') {
                    this.$message({
                        showClose: true,
                    ...
                    if (this.status === this.$t('approvalDetail.derate')) {
                        if(Number(this.approvedAmtEnd) > Number(startAmt)) {
                            this.$message({
                                showClose: true,
                                message: this.$t('approvalDetail.cue7'),
                                type: 'warning'
                            });
                        } else if (Number(this.approvedAmtEnd) < 10000 && this.approvedAmtEnd !== ''){
                            this.$message({
                                showClose: true,
                            ...
                            .then(() => {
                                ...
                                let msg1 = {};
                                msg1.orderId = this.orderId;
                                if (this.status1 === this.$t('approvalDetail.adopt')) {
                                    msg1.status = 2;
                                }else if (this.status1 === this.$t('approvalDetail.refuse')) {
                                    msg1.status = 4;
                                }else if(this.status1 === this.$t('approvalDetail.derate')) {
                                    msg1.status = 3;
                                    msg1.approvedAmt = Number(this.approvedAmtEnd);
                                }
                                msg1.finalCheckOpinion = this.checkOpinion1;
                                msg1.msgType = this.lanStatus;
                                finalApprovel(msg1) 
                                .then(response => {
                                    this.$router.push({
                                        path: '/trust/confirmation', 
                                        name: '业务审批确认'
                                    })
                                })
                                this.$message({
                        ...
                        if(Number(this.approvedAmtEnd) > Number(format)) {
                            this.$message({
                                showClose: true,
                                message: this.$t('approvalDetail.cue7'),
                                type: 'warning'
            ...
            back() {
                ...
                this.$router.push('/trust/confirmation')
            },
            //终审额度效验
            blurFnEnd() {
                if (this.status === this.$t('approvalDetail.derate')) {  
                    let msg = this.approvedAmt.replace(/\./g,'')
                    if(Number(this.approvedAmtEnd) > Number(msg)) {
                        this.$message({
                            showClose: true,
                            message: this.$t('approvalDetail.cue7'),
                            type: 'warning'
            ...
            getParams () {
                this.userId = this.apprDetailsEnd.userId;
                this.realName = this.apprDetailsEnd.realName;
            },
            getList() {
                //通话录
