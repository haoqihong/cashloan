...
import { page, getTask, returnTask } from 'api/collection/processing/index'
export default {
    data() {
        return {
        listLoading: false,
    ...
    computed: {
        ...mapGetters(["elements", "language", "processPage"])
    },
    methods: {
        //开始时间
        getSTimeStart(val) {
        ...
        getList() {
            ...
            this.$store.dispatch('saveProcessPage', { path: this.$route.path, listQuery: this.listQuery });
        ...
        //查询
        search() {
            if (this.processPage.has(this.$route.path)) {
                this.listQuery = this.processPage.get(this.$route.path)
            }
            this.getList();
        },
        //重置
        ...
        handleSelectionChange(val) {
            ...
            if (this.multipleSelection.length !== 0) {
                if (this.multipleSelection[0].status === 20 || this.multipleSelection[0].status === 100) {
        ...
        approvalFn() {
            if(this.multipleSelection.length === 1) { 
                this.$store.dispatch('setCollDetail', this.multipleSelection[0])
                this.$router.push({
                    path: '/collection/collDetail', 
                    name: '催收详情'
                })            
