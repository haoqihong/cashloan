        ...
        <div class='menu-search'> 
            <div class='menu-title'>
              <div class='searchItem'>
                ...
                <el-select v-model='listQuery.groupId' size='small'>
                  <el-option
...
import { page, getCollectGroup } from 'api/reportForm/RushReport/index'
...
export default {
  ...
  computed: {
    ...mapGetters(["elements", "rushReportPage"])
  },
  created() {
    //催收小组
    let group = {};
    ...
    .then(response => {
      this.collGroup = this.collGroup.concat(response.data)
    })
    //渠道下拉
    getChannels()
    .then(response => {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$store.dispatch('saveRushReportPage', { path: this.$route.path, listQuery: this.listQuery });
      page(this.listQuery)
        .then(response => {
    ...
    //查询
    search() {
      if (this.rushReportPage.has(this.$route.path)) {
        this.listQuery = this.rushReportPage.get(this.$route.path)
      }
      this.getList();
    },
    //重置
